// LaneMarkPlatformDlg.cpp : 实现文件
//

#include "stdafx.h"
#include "LaneMarkPlatform.h"
#include "LaneMarkPlatformDlg.h"
...
// CLaneMarkPlatformDlg 对话框




CLaneMarkPlatformDlg::CLaneMarkPlatformDlg(CWnd* pParent /*=NULL*/)
: CDialog(CLaneMarkPlatformDlg::IDD, pParent)
, m_frameNum(0.00)
, m_frameCount(0.00)
, m_step(5)
, isSeeking(false)
...
void CLaneMarkPlatformDlg::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
...
BEGIN_MESSAGE_MAP(CLaneMarkPlatformDlg, CDialog)
	ON_WM_SYSCOMMAND()
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	//}}AFX_MSG_MAP
	ON_BN_CLICKED(IDC_OPENPIC, &CLaneMarkPlatformDlg::OnBnClickedOpenpic)
	ON_BN_CLICKED(IDC_OPENVIDEO, &CLaneMarkPlatformDlg::OnBnClickedOpenvideo)
	ON_BN_CLICKED(IDC_NEXTFRAME, &CLaneMarkPlatformDlg::OnBnClickedNextframe)
	ON_BN_CLICKED(IDC_PREVFRAME, &CLaneMarkPlatformDlg::OnBnClickedPrevframe)
	ON_WM_MOUSEMOVE()
	ON_WM_LBUTTONDOWN()
	ON_WM_LBUTTONUP()
	ON_WM_RBUTTONDOWN()
	ON_BN_CLICKED(IDC_SAVE, &CLaneMarkPlatformDlg::OnBnClickedSave)
	ON_BN_CLICKED(IDC_SETSAVEPATH, &CLaneMarkPlatformDlg::OnBnClickedSetSavePath)

	ON_WM_MOUSEWHEEL()
	ON_BN_CLICKED(IDC_SETIMGPATH, &CLaneMarkPlatformDlg::OnBnClickedSetimgpath)
	ON_BN_CLICKED(IDC_NEXTIMG, &CLaneMarkPlatformDlg::OnBnClickedNextimg)
	ON_BN_CLICKED(IDC_OPENSVPATH, &CLaneMarkPlatformDlg::OnBnClickedOpensvpath)
	ON_WM_DROPFILES()
	ON_BN_CLICKED(IDC_SETTINGS, &CLaneMarkPlatformDlg::OnBnClickedSettings)
	ON_BN_CLICKED(IDC_CLEAR, &CLaneMarkPlatformDlg::OnBnClickedClear)
	ON_WM_LBUTTONDBLCLK()
END_MESSAGE_MAP()


// CLaneMarkPlatformDlg 消息处理程序

BOOL CLaneMarkPlatformDlg::OnInitDialog()
{
...
void CLaneMarkPlatformDlg::OnSysCommand(UINT nID, LPARAM lParam)
{
...
void CLaneMarkPlatformDlg::OnPaint()
{
	if (IsIconic())
	{
...
//显示。
HCURSOR CLaneMarkPlatformDlg::OnQueryDragIcon()
{
...
// 选择文件夹
CString CLaneMarkPlatformDlg::GetPath()
{
...
CString CLaneMarkPlatformDlg::FindFile(int FileType, CString dir)
{
	CString FileName = _T("");
...
int CLaneMarkPlatformDlg::ShowImage(Mat image, int nID, bool ifFlush)
{
	if (image.empty())
		return 0;
...
// 画矩形函数
void CLaneMarkPlatformDlg::DrawRect(int nID, CPoint pt)
{
...
// 重画最后定型的矩形
void CLaneMarkPlatformDlg::reDrawRect(int nID)
{
	CDC * pRDC = GetDlgItem(nID)->GetDC();	// 画图DC
	pRDC->SelectStockObject(NULL_BRUSH);	// 刷子透明
	CPen myPen(PS_SOLID, 1, RGB(0,255,0));	// 创建画笔
...
// 根据起始点和结束点生成标注框所用的矩形
cv::Rect CLaneMarkPlatformDlg::CreateRect(CPoint pt1, CPoint pt2, bool zoom)
{
...
// 创建单张标注截取的区域图像
void CLaneMarkPlatformDlg::CreateLabelImg(CPoint pt1, CPoint pt2)
{
	if (imgShow.empty())
	{
	...
	// 转换为Mat格式并写入队列
	Mat newLabelImg = cvarrToMat(img2);  // default additional arguments: don't copy data.
...
void CLaneMarkPlatformDlg::SaveLabel(int Mode)
{
	if (imgShow.empty())
	{
		AfxMessageBox(_T("请先打开图片或视频进行标注"),MB_OK,0);
...
// 清除当前所有标注矩形框和数据
void CLaneMarkPlatformDlg::ClearLabel()
{
	if (!imgShow.empty())
...
// 跳帧函数
void CLaneMarkPlatformDlg::JumpFrame(bool direct)
{
	if (!myCap.isOpened())
	{
...
int CLaneMarkPlatformDlg::OpenImage(CString currFileName)
{
	USES_CONVERSION;
...
int CLaneMarkPlatformDlg::OpenVideo(CString currFileName)
{
	USES_CONVERSION;
	char * pFileName = T2A(currFileName);
	if (myCap.isOpened())
...
// 打开单张图片按钮响应
void CLaneMarkPlatformDlg::OnBnClickedOpenpic()
{
...
// 打开视频按钮响应
void CLaneMarkPlatformDlg::OnBnClickedOpenvideo()
{
	// 初始化帧数为0
	SetDlgItemInt(IDC_FRAMENUM,0);
	// 打开视频文件
...
// 向后跳帧
void CLaneMarkPlatformDlg::OnBnClickedNextframe()
{
	JumpFrame(true);
}

// 向前跳帧
void CLaneMarkPlatformDlg::OnBnClickedPrevframe()
{
	JumpFrame(false);
}
...
// 保存按钮
void CLaneMarkPlatformDlg::OnBnClickedSave()
{
	if (saveMode>2)
...
// 清除当前标注框及数据
void CLaneMarkPlatformDlg::OnBnClickedClear()
{
	ClearLabel();
}
...
// 双击保存响应
void CLaneMarkPlatformDlg::OnLButtonDblClk(UINT nFlags, CPoint point)
{
	if (!fixedLabel)
		OnBnClickedSave();
...
// 右键清除当前标注框及数据
void CLaneMarkPlatformDlg::OnRButtonDown(UINT nFlags, CPoint point)
{
	OnBnClickedClear();
...
// 键盘映射部分操作
BOOL CLaneMarkPlatformDlg::PreTranslateMessage(MSG* pMsg)
{
...
// 设置指定保存目录路径
void CLaneMarkPlatformDlg::OnBnClickedSetSavePath()
{
...
// 设置指定图像所在文件夹
void CLaneMarkPlatformDlg::OnBnClickedSetimgpath()
{
	CString path = GetPath();
	// 用户选取文件夹，更新路径
	if (path!=_T(""))
...
// 鼠标滚轮映射跳帧方向
BOOL CLaneMarkPlatformDlg::OnMouseWheel(UINT nFlags, short zDelta, CPoint pt)
{
...
// 根据扩展名判断是否为图像文件
int CLaneMarkPlatformDlg::isImageFile()
{
	int dot,len;
	CString ext = _T("");
...
// 点击读取文件夹中下一张图像
void CLaneMarkPlatformDlg::OnBnClickedNextimg()
{
	if (m_imgsPath==_T(""))
	{
...
// 文件夹中下一张图片
void CLaneMarkPlatformDlg::NextImage()
{
...
// 直接打开保存标注的文件夹
void CLaneMarkPlatformDlg::OnBnClickedOpensvpath()
{
...
void CLaneMarkPlatformDlg::SaveTXT(CString txtpath, char* c, int d)
{
...
void CLaneMarkPlatformDlg::SaveWholeImg(int Mode)
{
...
// 响应拖动文件操作
void CLaneMarkPlatformDlg::OnDropFiles(HDROP hDropInfo)
{
	CString dropPath = _T("");
	wchar_t path[200];
...
// 弹出选项对话框进行参数设置
void CLaneMarkPlatformDlg::OnBnClickedSettings()
{
	// 将当前设置赋值进子对话框显示
	ReadINI(inipath);
...
// 方便写入INI文件的封装函数
void CLaneMarkPlatformDlg::WriteIntValue(char* lpAppName, char* lpKeyName, int Value, char* lpFileName)
{
	char ValBuf[16];
...
// 写入INI文件
void CLaneMarkPlatformDlg::WriteINI(char * path)
{
...
// 读取INI文件以获取配置参数
void CLaneMarkPlatformDlg::ReadINI(char * path)
{
